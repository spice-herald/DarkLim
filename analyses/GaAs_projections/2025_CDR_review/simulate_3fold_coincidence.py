...
GaAs_energy_resolution = 0.127
PD_energy_resolution = 0.223
n_sigma = 6.0  # Number of sigma for the trigger
...
def simulate_and_collect(sim_number):
    ...
    SE_PD.reset_sim()
    SE_PD.add_lee_bkgd_from_file(lee_input_file, scale_by=lee_scale)
    ...
    SE_GaAs.reset_sim()
    SE_GaAs.add_lee_bkgd_from_file(lee_input_file, scale_by=lee_scale)
    ...
    arrival_times_pix2 = np.cumsum(rng.exponential(1.0 / rate_Hz_PD, expected_events_PD)) * fs
    arrival_times_pix2 = arrival_times_pix2[arrival_times_pix2 < (sim_trace_length - f_frequencies)].astype(int)
    energies_pix2 = SE_PD.generate_background(E_max_PD, e_low=E_cutoff_PD) * 1000 # Convert to eV

    # Add pulses to pixel 2 waveform
    for i, arrival_time in enumerate(arrival_times_pix2):
        simulated_waveform_pix2_td[arrival_time:arrival_time + f_frequencies] += energies_pix2[i] * template_OF[2,2]
