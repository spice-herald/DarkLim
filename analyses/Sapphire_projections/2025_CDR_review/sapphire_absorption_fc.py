...
def process_mass(mass, args):
    ...
    dRdE_bkgd_arr /= args.LEE_improvement # Improvement factor for sapphire
    dRdE_bkgd_arr *= args.t_days # scale to exposure time; new units are cts/keV

    # Load signal template for bosonic absorption
    dRdE_signal_fun = darklim.elf.get_dRdE_lambda_Al2O3_absorption(mX_eV=mass*1e9, kappa=args.sigma0, res_eV=args.baseline_res_eV, suppress_darkelf_output=False)
...
def sapphire_scan():
    ...
    args.target = 'Al2O3'
    args.masses_GeV = np.append(np.geomspace(2e-11, 6e-10, 72), np.geomspace(5e-9, 150e-9, 72))
